...
func setupConfig() {
	...
	mu.RLock()
        defer mu.RUnlock()
	yamlPath := filepath.Join(WorkDir, "conf", "upstream_nodes.yaml")
        ...
        // 解析 YAML 文件
        var cfg UpstreamNodesConfig
        if err := yaml.Unmarshal(data, &cfg); err != nil {
                return
        }

	for _, host := range cfg.Hosts {
		fmt.Println("load ", host.IP)
		ip2Host[host.IP] = host.Hostname
		allHost = append(allHost, UpstreamNode{
...
func GetHosts() []UpstreamNode {
	mu.RLock()
        defer mu.RUnlock()
	ret := make([]UpstreamNode, len(allHost))
        copy(ret, allHost)
	return ret
}
...
func GetHostname(ip string) string {
	mu.RLock()
        defer mu.RUnlock()
	if val, ok := ip2Host[ip]; ok {
            return val
        }

	fmt.Println("xxxxx", ip)
	return "unknown"
}
